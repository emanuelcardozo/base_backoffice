...
export default function use{{cookiecutter.resource_name_singular}}Creation() {
  ...
  const { response, doFetch, loading, error } = useFetch(
    `${config.api.ms{{cookiecutter.resource_name}}.baseUrl}/{{cookiecutter.resource_name}}`,
    {
      method: 'POST',
    }
  )
  ...
  useEffect(() => {
    ...
    navigate('/{{cookiecutter.resource_name}}')

    const message = t('createdSuccessfully', {
      name: response.data.name,
      type: t('features:{{cookiecutter.resource_name}}:singular'),
    })
