...
export default function {{cookiecutter.resource_name_singular}}Edition() {
  ...
  const { {{cookiecutter.resource_name_singular}}, loading } = useFetch{{cookiecutter.resource_name_singular}}Detail(id, edition{{cookiecutter.resource_name_singular}}FromAPI)
  const { onSubmit, loading: isSubmitting } = use{{cookiecutter.resource_name_singular}}Edition(id)

  return (
    <Container maxWidth="xl">
      <Stack spacing={3} my={3}>
        <SectionBackButton label={t('listing.title')} to="/{{cookiecutter.resource_name}}" />
        <Stack
          direction={direction}
        ...
        <{{cookiecutter.resource_name_singular}}Form
          onSubmit={onSubmit}
          initialValues={ {{cookiecutter.resource_name_singular}} }
          mode="edit"
          isLoading={loading}
