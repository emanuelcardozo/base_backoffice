...
Hola
export default function HolaDetail() {
  const { id } = useParams()
  const { t } = useTranslation('features', { keyPrefix: 'Holas' })
  const { Hola, loading } = useFetchHolaDetail(id)
  const isLoading = loading || Hola === null
              ...
              {BASIC_DETAILS_FIELDS.map((fieldName) => (
                <ListItem key={fieldName} disableGutters>
                  <ListItemText
                    ...
                    secondary={<LoadingValue loading={isLoading} value={Hola?.[fieldName]} />}
                  />
                </ListItem>
              ))}
            </List>
