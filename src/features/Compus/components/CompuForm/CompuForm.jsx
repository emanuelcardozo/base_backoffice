...
import CompuSchema from 'features/Compus/schema/CompuSchema'
...
function CompuForm({ onCancel, onSubmit, initialValues, mode, isLoading, isSubmitting }) {
  const { t, i18n } = useTranslation(['features'], { keyPrefix: 'Compus.form' })
  ...
  const getFieldProps = useCallback(
    (name) => ({
      ...
      onClear: () => setFieldValue(name, EMPTY_COMPU[name]),
    }),
    ...
    <form noValidate onSubmit={handleSubmit}>
      <Stack spacing={3} sx={styles}>
        <CardFormBlock title={t('details')}>
          ...
          <TextField fullWidth label={t('marca')} {...getFieldProps('marca')} />

          <TextField fullWidth label={t('id')} {...getFieldProps('id')} />

          <TextField fullWidth label={t('linda')} {...getFieldProps('linda')} />
        </CardFormBlock>
...
CompuForm.defaultProps = {
  initialValues: EMPTY_COMPU,
  mode: 'create',
  isLoading: false,
  isSubmitting: false,
}
