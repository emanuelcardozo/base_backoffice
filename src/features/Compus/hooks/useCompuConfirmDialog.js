...
export default function useCompusConfirmDialog(refresh) {
  const { t } = useTranslation('features', { keyPrefix: 'Compu' })
  const confirm = useConfirm()
  const { doFetch } = useFetch()

  const onClickOpenConfirm = useCallback(
    (Compus) => {
      const action = Compus?.hidden ? 'unhide' : 'hide'
      ...
      confirm({
        ...
        .then(() => {
          doFetch({
            method: 'PATCH',
            url: `${config.api.msCompu.baseUrl}/Compu/${Compus.id}`,
            data: {
              active: Compus?.hidden,
            },
          }).then(refresh)
        })
        .catch(() => {
