...
export default function useJauriaConfirmDialog(refresh) {
  const { t } = useTranslation('features', { keyPrefix: 'Perro' })
  const confirm = useConfirm()
  const { doFetch } = useFetch()

  const onClickOpenConfirm = useCallback(
    (Jauria) => {
      const action = Jauria?.hidden ? 'unhide' : 'hide'
      ...
      confirm({
        ...
        .then(() => {
          doFetch({
            method: 'PATCH',
            url: `${config.api.msPerro.baseUrl}/Perro/${Jauria.id}`,
            data: {
              active: Jauria?.hidden,
            },
          }).then(refresh)
        })
        .catch(() => {
