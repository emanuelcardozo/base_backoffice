...
Perro
export default function PerroDetail() {
  const { id } = useParams()
  const { t } = useTranslation('features', { keyPrefix: 'Jauria' })
  const { Perro, loading } = useFetchPerroDetail(id)
  const isLoading = loading || Perro === null
              ...
              {BASIC_DETAILS_FIELDS.map((fieldName) => (
                <ListItem key={fieldName} disableGutters>
                  <ListItemText
                    ...
                    secondary={<LoadingValue loading={isLoading} value={Perro?.[fieldName]} />}
                  />
                </ListItem>
              ))}
            </List>
