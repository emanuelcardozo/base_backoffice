...
import PerroSchema from 'features/Jauria/schema/PerroSchema'
...
function PerroForm({ onCancel, onSubmit, initialValues, mode, isLoading, isSubmitting }) {
  const { t, i18n } = useTranslation(['features'], { keyPrefix: 'Jauria.form' })
  ...
  const getFieldProps = useCallback(
    (name) => ({
      ...
      onClear: () => setFieldValue(name, EMPTY_PERRO[name]),
    }),
    ...
    <form noValidate onSubmit={handleSubmit}>
      <Stack spacing={3} sx={styles}>
        <CardFormBlock title={t('details')}>
          ...
          <TextField fullWidth label={t('gustos')} {...getFieldProps('gustos')} />
        </CardFormBlock>
...
PerroForm.defaultProps = {
  initialValues: EMPTY_PERRO,
  mode: 'create',
  isLoading: false,
  isSubmitting: false,
}
