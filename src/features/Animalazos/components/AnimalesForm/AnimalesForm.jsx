...
import AnimalesSchema from 'features/Animalazos/schema/AnimalesSchema'

const direction = { xs: 'column-reverse', sm: 'row' }

const EMPTY_ANIMALES = {
  nombre: '',
...
function AnimalesForm({ onCancel, onSubmit, initialValues, mode, isLoading, isSubmitting }) {
  const { t, i18n } = useTranslation(['features'], { keyPrefix: 'Animalazos.form' })
  ...
  const {
    ...
    validationSchema: AnimalesSchema,
    onSubmit,
  })
    ...
    <form noValidate onSubmit={handleSubmit}>
      <Stack spacing={3} sx={styles}>
        <CardFormBlock title={t('details')}>
          ...
          <TextField fullWidth label={t('macho')} {...getFieldProps('macho')} />
        </CardFormBlock>
...
AnimalesForm.defaultProps = {
  initialValues: EMPTY_ANIMALES,
  mode: 'create',
  isLoading: false,
  isSubmitting: false,
}

AnimalesForm.propTypes = {
