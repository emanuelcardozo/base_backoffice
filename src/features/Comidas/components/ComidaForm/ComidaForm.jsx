...
import ComidaSchema from 'features/Comidas/schema/ComidaSchema'
...
function ComidaForm({ onCancel, onSubmit, initialValues, mode, isLoading, isSubmitting }) {
  const { t, i18n } = useTranslation(['features'], { keyPrefix: 'Comidas.form' })
  ...
  const getFieldProps = useCallback(
    (name) => ({
      ...
      onClear: () => setFieldValue(name, EMPTY_COMIDA[name]),
    }),
    ...
    <form noValidate onSubmit={handleSubmit}>
      <Stack spacing={3} sx={styles}>
        <CardFormBlock title={t('details')}>
          ...
          <TextField fullWidth label={t('fecha')} {...getFieldProps('fecha')} />

          <TextField fullWidth label={t('dia')} {...getFieldProps('dia')} />
        </CardFormBlock>
...
ComidaForm.defaultProps = {
  initialValues: EMPTY_COMIDA,
  mode: 'create',
  isLoading: false,
  isSubmitting: false,
}

ComidaForm.propTypes = {
