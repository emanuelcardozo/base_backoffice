...
export default function useDescuentosConfirmDialog(refresh) {
  const { t } = useTranslation('features', { keyPrefix: 'Descuento' })
  const confirm = useConfirm()
  const { doFetch } = useFetch()

  const onClickOpenConfirm = useCallback(
    (Descuentos) => {
      const action = Descuentos?.hidden ? 'unhide' : 'hide'
      ...
      confirm({
        ...
        .then(() => {
          doFetch({
            method: 'PATCH',
            url: `${config.api.msDescuento.baseUrl}/Descuento/${(Descuentos).id}`,
            data: {
              active: Descuentos?.hidden,
            },
          }).then(refresh)
        })
        .catch(() => {
