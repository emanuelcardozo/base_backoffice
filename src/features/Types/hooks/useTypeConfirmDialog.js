...
export default function useTypesConfirmDialog(refresh) {
  const { t } = useTranslation('features', { keyPrefix: 'Type' })
  const confirm = useConfirm()
  const { doFetch } = useFetch()

  const onClickOpenConfirm = useCallback(
    (Types) => {
      const action = Types?.hidden ? 'unhide' : 'hide'
      ...
      confirm({
        ...
        .then(() => {
          doFetch({
            method: 'PATCH',
            url: `${config.api.msType.baseUrl}/Type/${(Types).id}`,
            data: {
              active: Types?.hidden,
            },
          }).then(refresh)
