...
Moneda
export default function MonedaDetail() {
  const { id } = useParams()
  const { t } = useTranslation('features', { keyPrefix: 'Monedas' })
  const { Moneda, loading } = useFetchMonedaDetail(id)
  const isLoading = loading || Moneda === null
              ...
              {BASIC_DETAILS_FIELDS.map((fieldName) => (
                <ListItem key={fieldName} disableGutters>
                  <ListItemText
                    ...
                    secondary={<LoadingValue loading={isLoading} value={(Moneda)?.[fieldName]} />}
                  />
                </ListItem>
              ))}
            </List>
